...
class DataRepository {
  ...
  Future<List<QuizItem>> fetchBatch({int size = 10, int skip = 0}) async {
    try {
      final result =
          await _db.from('questions').select().range(skip, skip + size - 1);
  ...
  Future<Analytics> fetchAnalytics(String quizUid) async {
    try {
      final result = await _db.rpc('get_question_stats',
          params: {'question_id_param': int.parse(quizUid)});
    ...
    try {
      if (_deviceIdentifier != null) {
        ...
        await _db
            .from('user_profiles')
            .delete()
            .eq('device_id', _deviceIdentifier!);
      }
    } catch (error) {
